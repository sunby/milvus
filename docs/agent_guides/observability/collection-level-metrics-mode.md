...
本文记录 `common.metrics.collectionLevelMode` 对全部 71 个 collection / VChannel
级 Prometheus 指标族的影响，并逐项对比修改前（以及默认 `full` 模式）与
`aggregate` 模式的行为。其中 60 个指标族直接带 collection 标签，15 个直接带
...
| `aggregate` | 不导出真实 collection 或 VChannel 标签值；保留标签键，但将可安全合并的值统一写为 `all`。无法给出真实聚合语义的 Gauge 停止导出。 |
...
- 处理 `collection_id`、`collection_name`，以及经 writer 逐项确认的 VChannel
  标签 `channel_name`、`vchannel` 和 C++ cache `shard`。不会根据标签名盲目处理
  所有 `channel_name`；PChannel 指标保持原行为。
...
- Gauge 不允许依赖“最后一次 `Set` 恰好代表所有 collection”这种不稳定语义。
...
该指标不持久化 GaugeVec 时序，而是在 scrape 时从当前 Top-N 快照构造样本，
因此可以在输出边界安全替换 collection 标签。
...
| `milvus_datanode_growing_source_sync_failure_count` | 按 node、collection、channel 设置连续失败数。 | 不导出；它是 collection/channel 局部 `Set`，当前没有安全的跨 collection 聚合和清理生命周期。 |
...
| `milvus_querynode_level_zero_size` | 按 node、collection、channel 设置 L0 delete buffer 大小。 | 不导出；当前没有跨 collection 的快照聚合，折叠后也无法按 collection 安全清理 channel 时序。 |
...
- 已确认是 PChannel 的 `channel_name` 不处理，包括 RootCoord produce time-tick lag、
  ...
  Service / WAL 指标。`StreamingCoordVChannelTotal` 虽然统计 VChannel 数量，其
  `channel_name` 标签仍是 PChannel，也保持不变。
- 除 `internal_cache_shard_disk_usage_bytes` 外，C++ Milvus core、Knowhere、
  milvus-storage FFI 和 jemalloc 导出链没有直接 collection / VChannel 标签，行为不变。
