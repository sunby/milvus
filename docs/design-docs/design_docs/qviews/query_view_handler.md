...
| ApplyViews ordering | Preparing-first (protect segment ref counts) | Sequential |
...
5. `OnUnrecoverable` → UpRecovering → Unrecoverable (no report, no persist deletion).

**UpRecovering → Unrecoverable does NOT report to Coord.** The view is marked locally unavailable. Coord still believes it is Up. Discovery happens through the query path: the query planner detects the unavailable view and reports to Coord to generate a replacement. This avoids:
- Dependence on `OnReport` callback (which is nil until Coord re-pushes).
- Irreversible cleanup cascade for what may be a transient failure.
- Persisted Up info is retained for possible retry on SN restart.
...
The Unrecoverable case is critical: `OnUnrecoverable` from UpRecovering intentionally retains the persisted Up. The deletion is deferred to the Dropping transition triggered by Coord's Dropped push.
...
| `Recover` | Must eventually invoke exactly one of `OnRecoveringDone` or `OnUnrecoverable` |
| `Release` | Must eventually invoke `OnDropped` exactly once |

All callbacks must be asynchronous. Synchronous invocation during Acquire/Recover/Release will deadlock the shard mutex.
