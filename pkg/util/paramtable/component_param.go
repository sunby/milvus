...
type dataCoordConfig struct {
	...
	CompactionMaxTaskNum                   ParamItem `refreshable:"true"`
	CompactionMaxTaskNumPerTrigger         ParamItem `refreshable:"true"`
...
mix is prioritized by level: mix compactions first, then L0 compactions, then clustering compactions.`,
	...
	p.CompactionMaxTaskNum = ParamItem{
		Key:          "dataCoord.compaction.maxTaskNum",
		Version:      "3.0.0",
		DefaultValue: "1000",
		Doc:          "Maximum number of compaction tasks in pending or running state. Non-positive value means unlimited.",
		Export:       true,
	}
	p.CompactionMaxTaskNum.Init(base.mgr)

	p.CompactionMaxTaskNumPerTrigger = ParamItem{
		Key:          "dataCoord.compaction.maxTaskNumPerTrigger",
		Version:      "3.0.0",
		DefaultValue: "500",
		Doc:          "Maximum number of compaction tasks submitted by one trigger invocation. Non-positive value means unlimited.",
		Export:       true,
	}
	p.CompactionMaxTaskNumPerTrigger.Init(base.mgr)
