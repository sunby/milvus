...
func (s *MaintenanceSuite) TestPrewarm() {
	...
	s.Run("success", func() {
		...
		s.mock.EXPECT().Prewarm(mock.Anything, mock.Anything).RunAndReturn(func(ctx context.Context, req *milvuspb.PrewarmRequest) (*milvuspb.PrewarmResponse, error) {
			...
			s.Equal("low", req.GetPriority())
			s.EqualValues(3600, req.GetTtlSeconds())
		...
		result, err := s.client.Prewarm(ctx, NewPrewarmOption(collectionName, namespace).
			WithReplica(replicaNum).
			WithResourceGroup(rgs...).
			WithPriority("low").
			WithTTL(3600*time.Second))
		s.NoError(err)
		s.Equal("prewarm_10001", result.TaskID)
		s.Equal(namespace, result.Namespace)
	})
	...
	s.Run("failure", func() {
		...
		_, err := s.client.Prewarm(ctx, NewPrewarmOption(collectionName, namespace))
		s.Error(err)
	})
}

func (s *MaintenanceSuite) TestDescribePrewarmTask() {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	s.Run("success", func() {
		...
		s.mock.EXPECT().DescribePrewarmTask(mock.Anything, mock.Anything).RunAndReturn(func(ctx context.Context, req *milvuspb.DescribePrewarmTaskRequest) (*milvuspb.DescribePrewarmTaskResponse, error) {
		...
		result, err := s.client.DescribePrewarmTask(ctx, NewDescribePrewarmTaskOption(taskID))
		s.NoError(err)
		s.Equal(taskID, result.TaskID)
		s.Equal(milvuspb.PrewarmTaskState_PrewarmTaskStateWarming, result.State)
		s.EqualValues(60, result.Progress)
		s.Empty(result.ErrorMessage)
	})

	s.Run("failure", func() {
		taskID := "prewarm_missing"

		s.mock.EXPECT().DescribePrewarmTask(mock.Anything, mock.Anything).Return(nil, merr.WrapErrServiceInternal("mocked")).Once()

		_, err := s.client.DescribePrewarmTask(ctx, NewDescribePrewarmTaskOption(taskID))
		s.Error(err)
	})
}
