...
func (node *Proxy) CreateCollection(ctx context.Context, request *milvuspb.CreateCollectionRequest) (*commonpb.Status, error) {
	...
	metrics.ProxyDDLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method, metrics.TotalLabel).Inc()
	...
	if err := node.sched.ddQueue.Enqueue(cct); err != nil {
		...
		metrics.ProxyDDLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method, metrics.AbandonLabel).Inc()
		return &commonpb.Status{
			ErrorCode: commonpb.ErrorCode_UnexpectedError,
			Reason:    err.Error(),
		}, nil
	...
	if err := cct.WaitToFinish(); err != nil {
		...
		metrics.ProxyDDLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method, metrics.FailLabel).Inc()
		return &commonpb.Status{
			ErrorCode: commonpb.ErrorCode_UnexpectedError,
			Reason:    err.Error(),
		}, nil
	...
	metrics.ProxyDDLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method, metrics.SuccessLabel).Inc()
	metrics.ProxyDDLReqLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method).Observe(float64(tr.ElapseSpan().Milliseconds()))
...
func (node *Proxy) HasCollection(ctx context.Context, request *milvuspb.HasCollectionRequest) (*milvuspb.BoolResponse, error) {
	...
	tr := timerecord.NewTimeRecorder(method)
	metrics.ProxyDQLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method,
	...
	metrics.ProxyDQLReqLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method,
...
func (node *Proxy) LoadCollection(ctx context.Context, request *milvuspb.LoadCollectionRequest) (*commonpb.Status, error) {
	...
	if err := node.sched.ddQueue.Enqueue(lct); err != nil {
		...
		metrics.ProxyDMLFunctionCall.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method,
			request.CollectionName, metrics.AbandonLabel).Inc()
		return &commonpb.Status{
			ErrorCode: commonpb.ErrorCode_UnexpectedError,
			Reason:    err.Error(),
	...
	metrics.ProxyDMLReqLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), method,
...
func (node *Proxy) Insert(ctx context.Context, request *milvuspb.InsertRequest) (*milvuspb.MutationResult, error) {
	...
	if len(it.PartitionName) <= 0 {
		it.PartitionName = util.DefaultPartitionName
	}
	...
	if err := node.sched.dmQueue.Enqueue(it); err != nil {
		...
		metrics.ProxyInsertCount.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), request.CollectionName, metrics.AbandonLabel).Inc()
	...
	if err := it.WaitToFinish(); err != nil {
		...
		metrics.ProxyInsertCount.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
	...
	metrics.ProxyInsertVectors.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
		strconv.FormatInt(it.CollectionID, 10)).Add(float64(it.result.InsertCnt))
	metrics.ProxyInsertLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
...
func (node *Proxy) Search(ctx context.Context, request *milvuspb.SearchRequest) (*milvuspb.SearchResults, error) {
	...
	qt := &searchTask{
		...
		SearchRequest: &internalpb.SearchRequest{
			...
			ResultChannelID: node.queryResultChannel,
		},
	...
	if err := node.sched.dqQueue.Enqueue(qt); err != nil {
		...
		metrics.ProxySearchCount.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
	...
	if err := qt.WaitToFinish(); err != nil {
		...
		metrics.ProxySearchCount.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10), request.CollectionName, metrics.SearchLabel, metrics.FailLabel).Inc()
	...
	metrics.ProxySearchVectors.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
		strconv.FormatInt(qt.CollectionID, 10), metrics.SearchLabel).Set(float64(qt.result.Results.NumQueries))
	searchDur := tr.ElapseSpan().Milliseconds()
	metrics.ProxySearchLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
		strconv.FormatInt(qt.CollectionID, 10), metrics.SearchLabel).Observe(float64(searchDur))
	metrics.ProxySearchLatencyPerNQ.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
...
func (node *Proxy) Query(ctx context.Context, request *milvuspb.QueryRequest) (*milvuspb.QueryResults, error) {
	...
	qt := &queryTask{
		...
		RetrieveRequest: &internalpb.RetrieveRequest{
			...
			ResultChannelID: strconv.FormatInt(node.session.ServerID, 10),
		},
	...
	metrics.ProxySendMessageLatency.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10),
...
func (node *Proxy) RegisterLink(ctx context.Context, req *milvuspb.RegisterLinkRequest) (*milvuspb.RegisterLinkResponse, error) {
	...
	metrics.ProxyLinkedSDKs.WithLabelValues(strconv.FormatInt(node.session.ServerID, 10)).Inc()
	return &milvuspb.RegisterLinkResponse{
		Address: nil,
		Status: &commonpb.Status{
			ErrorCode: commonpb.ErrorCode_Success,
...
func (node *Proxy) GetMetrics(ctx context.Context, req *milvuspb.GetMetricsRequest) (*milvuspb.GetMetricsResponse, error) {
	log.Debug("Proxy.GetMetrics",
		zap.Int64("node_id", node.session.ServerID),
	...
	if !node.checkHealthy() {
		log.Warn("Proxy.GetMetrics failed",
			zap.Int64("node_id", node.session.ServerID),
			zap.String("req", req.Request),
			zap.Error(errProxyIsUnhealthy(node.session.ServerID)))

		return &milvuspb.GetMetricsResponse{
			Status: &commonpb.Status{
				ErrorCode: commonpb.ErrorCode_UnexpectedError,
				Reason:    msgProxyIsUnhealthy(node.session.ServerID),
			},
			Response: "",
		}, nil
	}
...
func (node *Proxy) LoadBalance(ctx context.Context, req *milvuspb.LoadBalanceRequest) (*commonpb.Status, error) {
	log.Debug("Proxy.LoadBalance",
		zap.Int64("proxy_id", node.session.ServerID),
		zap.Any("req", req))
