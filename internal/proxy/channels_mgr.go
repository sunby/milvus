...
func (mgr *singleTypeChannelsMgr) createMsgStream(collectionID UniqueID) error {
	...
	for _, pc := range pchans {
		metrics.ProxyMsgStreamObjectsForPChan.WithLabelValues(strconv.FormatInt(ServerID, 10), pc).Inc()
	}
	return nil
}
...
func (mgr *singleTypeChannelsMgr) removeStream(collectionID UniqueID) error {
	...
	for _, pc := range channels {
		metrics.ProxyMsgStreamObjectsForPChan.WithLabelValues(strconv.FormatInt(ServerID, 10), pc).Dec()
	}
	return nil
}
...
func (mgr *channelsMgrImpl) createDQLStream(collectionID UniqueID) error {
	metrics.ProxyMsgStreamObjectsForSearch.WithLabelValues(strconv.FormatInt(ServerID, 10), "query").Inc()
...
func (mgr *channelsMgrImpl) removeDQLStream(collectionID UniqueID) error {
	metrics.ProxyMsgStreamObjectsForSearch.WithLabelValues(strconv.FormatInt(ServerID, 10), "query").Dec()
