...
func (it *IndexBuildTask) loadVector(ctx context.Context) (storage.FieldID, storage.FieldData, error) {
	...
	metrics.IndexNodeLoadBinlogLatency.WithLabelValues(strconv.FormatInt(serverID, 10)).Observe(float64(loadVectorDuration))
	metrics.IndexNodeDecodeBinlogLatency.WithLabelValues(strconv.FormatInt(serverID, 10)).Observe(float64(it.tr.RecordSpan()))
...
func (it *IndexBuildTask) buildIndex(ctx context.Context) ([]*storage.Blob, error) {
	var fieldID storage.FieldID
	{
		...
		metrics.IndexNodeKnowhereBuildIndexLatency.WithLabelValues(strconv.FormatInt(serverID, 10)).Observe(float64(it.tr.RecordSpan()))
	...
	metrics.IndexNodeEncodeIndexFileLatency.WithLabelValues(strconv.FormatInt(serverID, 10)).Observe(float64(encodeIndexFileDur.Milliseconds()))
...
func (it *IndexBuildTask) saveIndex(ctx context.Context, blobs []*storage.Blob) error {
	...
	getSavePathByKey := func(key string) string {
		return path.Join(util.GetPath(it.cfg, util.IndexStorageRoot), strconv.Itoa(int(it.req.IndexBuildID)), strconv.Itoa(int(it.req.Version)),
...
func (it *IndexBuildTask) Execute(ctx context.Context) error {
	...
	metrics.IndexNodeSaveIndexFileLatency.WithLabelValues(strconv.FormatInt(serverID, 10)).Observe(float64(saveIndexFileDur.Milliseconds()))
