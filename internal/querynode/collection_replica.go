...
func (colReplica *collectionReplica) addCollection(collectionID UniqueID, schema *schemapb.CollectionSchema) *Collection {
	...
	metrics.QueryNodeNumCollections.WithLabelValues(fmt.Sprint(queryNodeID)).Set(float64(len(colReplica.collections)))
...
func (colReplica *collectionReplica) removeCollectionPrivate(collectionID UniqueID) error {
	...
	metrics.QueryNodeNumCollections.WithLabelValues(fmt.Sprint(queryNodeID)).Set(float64(len(colReplica.collections)))
	metrics.QueryNodeNumPartitions.WithLabelValues(fmt.Sprint(queryNodeID)).Sub(float64(len(collection.partitionIDs)))
	return nil
}
...
func (colReplica *collectionReplica) addPartitionPrivate(collectionID UniqueID, partitionID UniqueID) error {
	...
	metrics.QueryNodeNumPartitions.WithLabelValues(fmt.Sprint(queryNodeID)).Set(float64(len(colReplica.partitions)))
	return nil
}
...
func (colReplica *collectionReplica) addSegmentPrivate(segmentID UniqueID, partitionID UniqueID, segment *Segment) error {
	...
	metrics.QueryNodeNumSegments.WithLabelValues(fmt.Sprint(queryNodeID)).Inc()
	return nil
}
...
func (colReplica *collectionReplica) removeSegmentPrivate(segmentID UniqueID) error {
	...
	metrics.QueryNodeNumSegments.WithLabelValues(fmt.Sprint(queryNodeID)).Dec()
	return nil
}
