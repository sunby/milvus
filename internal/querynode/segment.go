...
func (s *Segment) search(plan *SearchPlan,
	...
	*/
	...
	metrics.QueryNodeSQSegmentLatencyInCore.WithLabelValues(fmt.Sprint(queryNodeID), metrics.SearchLabel).Observe(float64(tr.ElapseSpan().Milliseconds()))
...
func (s *Segment) retrieve(plan *RetrievePlan) (*segcorepb.RetrieveResults, error) {
	...
	metrics.QueryNodeSQSegmentLatencyInCore.WithLabelValues(fmt.Sprint(queryNodeID),
