...
TEST_P(TestChunkSegmentStorageV2, LazyManifestPreservesInitialMultiFieldTask) {
    ...
    auto int64_column = snapshot->runtime->fields.at(fields.at("int64"));
    auto string_column = snapshot->runtime->fields.at(fields.at("string1"));
    auto pk_column = snapshot->runtime->fields.at(fields.at("pk"));
    ...
    EXPECT_EQ(segment_impl->GetMemoryUsageInBytes(), memory_before_materialize);
    auto dropped_snapshot = segment_impl->TestGetPublishedStateSnapshot();
    ASSERT_NE(dropped_snapshot, snapshot);
    ASSERT_NE(dropped_snapshot->runtime, nullptr);
    EXPECT_EQ(dropped_snapshot->runtime->fields.count(fields.at("int64")), 0);
    ...
    auto commit_timestamp_column =
        commit_snapshot->runtime->fields.at(TimestampFieldID);
...
TEST_P(TestChunkSegmentStorageV2,
    ...
    auto column = snapshot->runtime->fields.at(field_id);
...
TEST_P(TestChunkSegmentStorageV2, LazyManifestReopenRebindsGeneration) {
    ...
    auto dropped_snapshot = segment_impl->TestGetPublishedStateSnapshot();
    ASSERT_NE(dropped_snapshot, new_snapshot);
    ASSERT_NE(dropped_snapshot->runtime, nullptr);
    EXPECT_EQ(dropped_snapshot->runtime->fields.count(fields.at("string1")), 0);
