...
TEST_P(RetrieveOutputFieldsTest, ParallelReadsPreserveOrderIDsAndStorageCost) {
    ...
    segment->before_fetch = [&](FieldId field_id, milvus::OpContext* ctx) {
        ...
        if (field_id == first) {
            // A serial field loop cannot reach last while first is waiting.
            EXPECT_EQ(last_ready.wait_for(5s), std::future_status::ready);
        } else if (field_id == last) {
...
TEST_P(RetrieveOutputFieldsTest, FailureCancelsAndDrainsSiblingsBeforeReturn) {
    plan->field_ids_ = {first, last};
    std::promise<void> first_started;
    auto first_ready = first_started.get_future();
    std::promise<void> sibling_cancelled;
    auto cancelled = sibling_cancelled.get_future();
    ...
    EXPECT_EQ(cancelled.wait_for(5s), std::future_status::ready);
    EXPECT_EQ(pending.wait_for(0s), std::future_status::timeout);
    release_first.set_value();
    try {
        pending.get();
        FAIL() << "Expected the original field read error";
...
TEST_P(RetrieveOutputFieldsTest, CancellationReachesRunningField) {
    ...
    try {
        pending.get();
        FAIL() << "Expected cancellation during the field read";
    } catch (const SegcoreError& error) {
        EXPECT_EQ(error.get_error_code(), ErrorCode::FollyCancel);
    }
}
