		...
		case hasV3:
			...
			mutations[segID] = append(mutations[segID], func(seg *datapb.SegmentInfo) bool {
				if seg.GetManifestPath() == "" {
					log.Ctx(ctx).Warn("batch update manifest version skipped: no manifest path",
						zap.Int64("segmentID", segID))
					return false
				}
				basePath, currentVer, err := packed.UnmarshalManifestPath(seg.GetManifestPath())
				if err != nil {
					log.Ctx(ctx).Warn("batch update manifest version skipped: invalid manifest path",
						zap.Int64("segmentID", segID), zap.Error(err))
					return false
				}
				if currentVer >= manifestVersion {
					if currentVer > manifestVersion {
						log.Ctx(ctx).Warn("batch update manifest version skipped: version regression",
							zap.Int64("segmentID", segID),
