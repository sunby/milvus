...
func (s *Server) SaveBinlogPaths(ctx context.Context, req *datapb.SaveBinlogPathsRequest) (*commonpb.Status, error) {
	...
	if req.GetSegLevel() == datapb.SegmentLevel_L0 {
		...
		log.Info("SaveBinlogPaths get segment done", zap.Duration("getSegmentDur", getSegmentDur))
		...
		if req.GetDropped() {
			...
			log.Info("SaveBinlogPaths drop segment done", zap.Duration("dropSegmentDur", dropSegmentDur))
		}
	...
	if validationSkipped {
		log.Info("SaveBinlogPaths validation skipped write",
	...
	log.Info("SaveBinlogPaths done",
		zap.Duration("total", time.Since(start)),
		zap.Duration("validate", validateDur),
		zap.Duration("buildMutations", buildMutationsDur),
		zap.Duration("updateSegmentsInfo", updateSegmentsInfoDur),
		zap.Duration("setLastWrittenTime", setLastWrittenTimeDur),
		zap.Any("checkpoints", req.GetCheckPoints()),
	)
