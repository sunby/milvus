...
func (s *Server) GetTimeTickChannel(ctx context.Context) (*milvuspb.StringResponse, error) {
	return &milvuspb.StringResponse{
		...
		Value: util.GetPath(s.cfg, util.DataCoordTimeTickChannel),
	}, nil
}
...
func (s *Server) GetSegmentInfoChannel(ctx context.Context) (*milvuspb.StringResponse, error) {
	return &milvuspb.StringResponse{
		...
		Value: util.GetPath(s.cfg, util.DataCoordSegmentInfoChannel),
	}, nil
}
...
func (s *Server) SaveBinlogPaths(ctx context.Context, req *datapb.SaveBinlogPathsRequest) (*commonpb.Status, error) {
	...
	if req.GetFlushed() {
		...
		if s.cfg.Compaction.EnableCompaction {
			...
			tt, err := getTimetravelReverseTime(cctx, s.cfg.RetentionDuration, s.allocator)
			if err == nil {
...
func (s *Server) GetMetrics(ctx context.Context, req *milvuspb.GetMetricsRequest) (*milvuspb.GetMetricsResponse, error) {
	...
	if s.isClosed() {
		log.Warn("DataCoord.GetMetrics failed",
			...
			zap.Error(errDataCoordIsUnhealthy(serverID)))

		return &milvuspb.GetMetricsResponse{
			Status: &commonpb.Status{
				ErrorCode: commonpb.ErrorCode_UnexpectedError,
				Reason:    msgDataCoordIsUnhealthy(serverID),
			},
			Response: "",
		}, nil
	}
...
func (s *Server) CompleteCompaction(ctx context.Context, req *datapb.CompactionResult) (*commonpb.Status, error) {
	...
	if s.isClosed() {
		...
		resp.Reason = msgDataCoordIsUnhealthy(serverID)
		return resp, nil
	}
...
func (s *Server) ManualCompaction(ctx context.Context, req *milvuspb.ManualCompactionRequest) (*milvuspb.ManualCompactionResponse, error) {
	...
	if s.isClosed() {
		log.Warn("failed to execute manual compaction", zap.Int64("collectionID", req.GetCollectionID()),
			zap.Error(errDataCoordIsUnhealthy(serverID)))
		resp.Status.Reason = msgDataCoordIsUnhealthy(serverID)
		return resp, nil
	}
	...
	tt, err := getTimetravelReverseTime(ctx, s.cfg.RetentionDuration, s.allocator)
	if err != nil {
...
func (s *Server) GetCompactionStateWithPlans(ctx context.Context, req *milvuspb.GetCompactionPlansRequest) (*milvuspb.GetCompactionPlansResponse, error) {
	...
	if s.isClosed() {
		log.Warn("failed to get compaction state with plans", zap.Int64("compactionID", req.GetCompactionID()), zap.Error(errDataCoordIsUnhealthy(serverID)))
		resp.Status.Reason = msgDataCoordIsUnhealthy(serverID)
		return resp, nil
	}
