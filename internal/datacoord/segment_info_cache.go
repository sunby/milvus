...
// backed by Cache. It provides secondary indexes for fast lookups by
// collection, channel, and compaction relationships.
...
// --- Local-only updates (no version bump, not persisted) ---
...
// updateSegment applies a local-only update (no version bump, not persisted).
