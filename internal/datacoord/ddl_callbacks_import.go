...
func (c *DDLCallbacks) commitImportV2AckCallback(ctx context.Context, result message.BroadcastResultCommitImportMessageV2) error {
	...
	var allSegIDs []int64
	mutations := make(map[int64][]MutateFunc)
	for vchannel, appendResult := range result.Results {
		...
		for _, segID := range segIDs {
			mutations[segID] = append(mutations[segID], UpdateCommitTimestamp(segID, commitTs)[segID]...)
			mutations[segID] = append(mutations[segID], UpdateIsImporting(segID, false)[segID]...)
		}
	}
	if len(mutations) > 0 {
