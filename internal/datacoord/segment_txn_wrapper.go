...
func (t *SegmentTxn) Insert(key string, seg *datapb.SegmentInfo) error {
	value, binlogKvs, err := buildSegmentWrite(seg, BinlogIncrement{
...
func (t *SegmentTxn) Update(key string, seg *datapb.SegmentInfo, expectedVersion int64, inc BinlogIncrement) error {
	value, binlogKvs, err := buildSegmentWrite(seg, inc)
	if err != nil {
		return err
	}
...
func buildSegmentWrite(seg *datapb.SegmentInfo, inc BinlogIncrement) ([]byte, map[string][]byte, error) {
	stripped := proto.Clone(seg).(*datapb.SegmentInfo)
	datacoordkv.ResetBinlogFields(stripped)
	value, err := proto.Marshal(stripped)
	if err != nil {
		return nil, nil, fmt.Errorf("marshal SegmentInfo: %w", err)
	}
	if inc.IsEmpty() {
