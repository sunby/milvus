...
func (s *Server) getSystemInfoMetrics(
	...
	coordTopology := metricsinfo.DataCoordTopology{
		...
		Connections: metricsinfo.ConnTopology{
			Name: metricsinfo.ConstructComponentName(typeutil.DataCoordRole, serverID),
	...
	resp := &milvuspb.GetMetricsResponse{
		...
		Response:      "",
		ComponentName: metricsinfo.ConstructComponentName(typeutil.DataCoordRole, serverID),
	}
	var err error
...
func (s *Server) getDataCoordMetrics() metricsinfo.DataCoordInfos {
	ret := metricsinfo.DataCoordInfos{
		BaseComponentInfos: metricsinfo.BaseComponentInfos{
			...
			CreatedTime: s.createdTime.String(),
			UpdatedTime: s.updatedTime.String(),
			Type:        typeutil.DataCoordRole,
			ID:          s.session.ServerID,
		},
		SystemConfigurations: metricsinfo.DataCoordConfiguration{
			SegmentMaxSize: float64(s.cfg.SegmentMaxSizeInMB),
		},
	}
