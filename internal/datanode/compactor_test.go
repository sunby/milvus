...
func TestCompactionTaskInnerMethods(t *testing.T) {
	...
	t.Run("Test merge", func(t *testing.T) {
		...
		ct := &compactionTask{}
		idata, err := ct.merge(mitr, dm, meta.GetSchema())
		assert.NoError(t, err)
		assert.Equal(t, 1, idata.Data[0].Length())
...
func TestCompactorInterfaceMethods(t *testing.T) {
	notEmptySegmentBinlogs := []*datapb.CompactionSegmentBinlogs{&datapb.CompactionSegmentBinlogs{
		SegmentID:           100,
		FieldBinlogs:        nil,
	...
	t.Run("Test typeI compact valid", func(t *testing.T) {
		...
		cpaths, err := mockbIO.upload(context.TODO(), segID, partID, iData, dData, meta)
		require.NoError(t, err)
		...
		plan.Timetravel = Timestamp(30000)
		err = task.compact()
		assert.NoError(t, err)
	...
	t.Run("Test typeII compact valid", func(t *testing.T) {
		...
		cpaths1, err := mockbIO.upload(context.TODO(), segID1, partID, iData1, dData1, meta)
		require.NoError(t, err)
		require.Equal(t, 11, len(cpaths1.inPaths))

		cpaths2, err := mockbIO.upload(context.TODO(), segID2, partID, iData2, dData2, meta)
		require.NoError(t, err)
		...
		plan.Timetravel = Timestamp(40000)
		replica.addFlushedSegmentWithPKs(segID1, collID, partID, "channelname", 2, []UniqueID{1})
		replica.addFlushedSegmentWithPKs(segID2, collID, partID, "channelname", 2, []UniqueID{9})
		replica.removeSegment(19530)
		require.True(t, replica.hasSegment(segID1, true))
