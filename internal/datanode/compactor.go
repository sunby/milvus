...
func (t *compactionTask) merge(mergeItr iterator, delta map[UniqueID]Timestamp, schema *schemapb.CollectionSchema) (*InsertData, error) {
	...
	for fID, content := range fID2Content {
		...
		fieldData, err := interface2FieldData(tp, content, numRows)
		if err != nil {
			log.Warn("transfer interface to FieldData wrong", zap.Error(err))
			return nil, err
		}
		iData.Data[fID] = fieldData
	}
...
func (t *compactionTask) compact() error {
	...
	iData, err := t.merge(mergeItr, deltaMap, meta.GetSchema())
	if err != nil {
		log.Error("compact wrong", zap.Int64("planID", t.plan.GetPlanID()), zap.Error(err))
		return err
	}

	cpaths, err := t.upload(ctxTimeout, targetSegID, partID, iData, deltaBuf.delData, meta)
	if err != nil {
		log.Error("compact wrong", zap.Int64("planID", t.plan.GetPlanID()), zap.Error(err))
		return err
	}
	...
	// numofRows are calculated from rowID field, fieldID 0
	numRows := int64(iData.Data[0].Length())
	...
	fd := iData.Data[0].(*storage.Int64FieldData).Data
